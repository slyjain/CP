...
int rec(int level,int x){
    ...
    int ans=0;
    // if(w[i]<=x){
    //     ans=max(ans,rec(level+1,x-w[i])+v[i]);
    // }
    for(int number_of_times=0;number_of_times<=x/w[i];number_of_times++){
        ans=max(ans,rec(level+1,x-number_of_times*w[level])+v[i]*number_of_times);
    }
    //save and return 
